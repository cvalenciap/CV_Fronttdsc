...
import {BandejaEntradaRecibidosService} from '../../../services/impl/bandeja-entrada-recibidos.service';
import {Documento, FiltroDocumento, OpcionBusqueda, Paginacion, Response} from '../../../models';
...
import {FileServerService} from '../../../services';
...
@Component({
  selector: 'bandeja-recibidos',
  templateUrl: 'bandeja-recibidos.template.html',
  styleUrls: ['bandeja-recibidos.component.scss']
})
export class BandejaRecibidosComponent implements OnInit {
  static item;
  items: Documento[];
  selectedRow: number;
  ...
  private SESSION_KEY_FILTRO = 'bandeja-recibidos.filtro';
  ...
  OnRestablecer() {
    ...
    this.textoInfoBusqueda = 'inicio';
    this.getDocumentos();
    // this.OnBusquedaAvanzada(this.filtro);
  }
  ...
  OnExcel() {
      ...
      this.bandejaService.generarExcel(this.filtro.values).subscribe(
        (data)=>{
          ...
          this_.fileService.downloadFile(file, 'bandeja-recibidos.xlsx');
          this.loading=false;
        },
  ...
  OnPDF() {
      ...
      this.bandejaService.generarPDF(this.filtro.values).subscribe(
        (data)=> {
          ...
          this_.urlReportePdf = URL.createObjectURL(file);
  ...
  OnLeido(item: Documento) {
    if (item.nindleido === 0) {
      this.bandejaService.actualizarLeido(item).subscribe((response: Response) => {
          console.log('Se ha actualizado el registro a leído', response);
        },
        (error) => this.controlarError(error)
      );
    }
