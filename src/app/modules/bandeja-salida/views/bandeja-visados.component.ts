...
import {BandejaSalidaVisadosService as BandejaSalidaVisadosService, BandejaSalidaService, FileServerService} from '../../../services';
...
@Component({
  selector: 'bandeja-visados',
  templateUrl: 'bandeja-visados.template.html',
  styleUrls: ['bandeja-visados.component.scss']
})
export class BandejaVisadosComponent implements OnInit {
  static item;
  items: Documento[];
  selectedRow: number;
  ...
  private SESSION_KEY_FILTRO = 'bandeja-visados.filtro';
  private SESSION_KEY_PAGINACION = 'bandeja-visados.paginacion';
  public textoInfoBusqueda: string;

 constructor(private router: Router, private toastr: ToastrService, public session: SessionService, private fileService: FileServerService,
    private bandejaService: BandejaSalidaVisadosService, private bandejaSalidaService: BandejaSalidaService) {
 }
  ...
  private inicializarVariables(): void {
    ...
    this.listaVisadoMultiple = [];
    this.bandeja = 'VISADOS';
    this.listaArchivos = [];
    this.tipoFirma = 'VB';
    this.selectedRow = -1;
    this.loading = true;
      ...
      this.bandejaService.generarExcel(this.filtro.values).subscribe(
        (data)=>  {
          ...
          /*if (window.navigator && window.navigator.msSaveOrOpenBlob) {
            window.navigator.msSaveOrOpenBlob(file, 'Reporte-Visados.xlsx');
          } else {
            window.open(fileURL);
          }*/
      },
  ...
  OnCheck(event) {
    event.stopPropagation();
    // event.item.selected = event.target.checked;
  }
    ...
    for (const item of this.items) {
      if (item.selected) {
        if (item.estado !== 'PENDIENTE') {
          ...
          this.listaVisadoMultiple.push(item);
        }
      }
    }
    if (i > 0) {
      this.toastr.warning('Para completar esta acción no debe seleccionar registros visados u observados',
  ...
  OnFirmaDigitalIniciar() {
    this.listaArchivos = this.listaVisadoMultiple.map((item) => { return item.urlDocumento; });
  ...
  OnFirmaDigitalFinalizar(e: any) {
    ...
    if (e.ok) {
      ...
      if (this.listaVisadoMultiple.length > 0) {
        this.bandejaSalidaService.visarDocumentos(this.listaVisadoMultiple).subscribe(
          (response: Response) => {
             if (response.estado = 'OK') {
               this.toastr.success('Los documentos seleccionados fueron visados correctamente', 'Acción completada!', {closeButton: true});
               this.getDocumentos();
  ...
  OnObservar() {
    let i = 0;
    const observadoMultiple: Documento[] = [];

    for (const item of this.items) {
      if (item.selected === true) {
        if (item.estado !== 'PENDIENTE') {
          this.toastr.warning('No debe seleccionar registros ya visados, ni observados', 'Acción completada!', {closeButton: true});
          return;
        }
  ...
  Limpiar() {
    ...
    this.filtroAvanzado.tipoDocumento=null;
    this.filtroAvanzado.numeroDocumento = null
    this.filtroAvanzado.referencia ="";
    this.filtroAvanzado.origen="0";
    this.filtroAvanzado.prioridad="0"
    this.filtroAvanzado.remitente=null;
    this.filtroAvanzado.dirigido=null;
